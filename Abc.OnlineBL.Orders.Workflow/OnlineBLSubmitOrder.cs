...
{
    public sealed partial class OnlineBLSubmitOrder : SequentialWorkflowActivity
    {
            ...
            try
            {
                ...
                if (!isStockBoardOrder)
                {
                    if ((orderDataExchange.PropertyOrder.OrderHasBoard() || (orderDataExchange.PropertyOrder.OrderHasBoardOverlay() && orderDataExchange.PropertyOrder.ClientHasStockboardDIYPreference) || orderDataExchange.PropertyOrder.OrderHasBrochure()
                        || orderDataExchange.PropertyOrder.OrderHasWindowCard() || orderDataExchange.PropertyOrder.OrderHasCorflute())
        ...
        RetryCreate:
            if (OnlineBLConfig.IS_NZ)
            {
                try
                {
                    ...
                    file.WriteAllText(fileNameToUse, contents);
                    ret = true;
                }
                catch (Exception ex)
                {
        ...
        private void GenerateOrderEventWrapper(bool forClient, bool forAbc, bool forB2B)
        {
            try
            {
                ...
                if (forAbc && orderDataExchange.OrderId > 0 && orderDataExchange.PropertyOrder.IsDIYOrder
                    ...
                    || orderDataExchange.PropertyOrder.OrderHasWindowCard() || orderDataExchange.PropertyOrder.OrderHasCorflute())
                    && string.IsNullOrEmpty(orderDataExchange.PropertyOrder.Notes) && string.IsNullOrEmpty(orderDataExchange.Client.Notes))
                {
                ...
                else
                {
                    ...
                    if (forAbc && !orderDataExchange.PropertyOrder.IsDIYOrder && !orderDataExchange.PropertyOrder.OrderHasBoard() && !orderDataExchange.PropertyOrder.OrderHasStockboardIncludePackageCheck()
                        && (orderDataExchange.PropertyOrder.OrderHasSoldSticker() || orderDataExchange.PropertyOrder.OrderHasOverlay()))
                    {
                        //check stockboard DIY order
                        if (forAbc && orderDataExchange.PropertyOrder.ClientHasStockboardDIYPreference && orderDataExchange.PropertyOrder.IsDIYOrder && !orderDataExchange.PropertyOrder.OrderHasBoard() && (orderDataExchange.PropertyOrder.OrderHasBoardOverlay() || orderDataExchange.PropertyOrder.OrderHasStockboardIncludePackageCheck()))
                        {
        ...
        {
            e.Result = orderDataExchange.PropertyOrder.OrderHasSpotlight();
        }

        private void CreateSpotlightFile_ExecuteCode(object sender, EventArgs e)
        {
            int id = orderDataExchange.OrderId;

            System.IO.StreamWriter sw = null;
            string property = orderDataExchange.Property.PropertyAddressWithSuburb;
            FormatOrder formatOrder = new FormatOrder(orderDataExchange, id);
            ...
            fileName = fileName.Replace('>', ' ');
            fileName = fileName.Replace('|', ' ');
            fileName = Path.Combine(OnlineBLConfig.SPOT_ORDER_FILE_DIR, fileName + ".rtf");
            ...
            {
                Logger.Exception(ex, string.Format("{0} {1}"), new object[] { orderDataExchange.PropertyOrder.ClientId, property, txt, id });
            }
        }
        ...
        private void UpdateAugment_ExecuteCode(object sender, EventArgs e)
        {
            if (orderDataExchange.PhotoOrderId > 0 && orderDataExchange.PropertyOrder.OrderHasAugmentPhotographyProduct())
            {
                OrderProcessor.GenerateAugmentServiceQueue(orderDataExchange.PhotoOrderId);
            }
        }

        private void ApplySurcharge_ExecuteCode(object sender, EventArgs e)
        {
            if (orderDataExchange.PropertyOrder.ApplySurcharge)
            {
                Logger.Warn("Apply Surcharge: " + orderDataExchange.OrderId);
                if (orderDataExchange.OrderId > 0)
                    OrderProcessor.GenerateOrderSurcharge(orderDataExchange.OrderId);
                if (orderDataExchange.StockId > 0)
                {
                    if (orderDataExchange.StockId < 99000000)
                        OrderProcessor.GenerateOrderSurcharge(orderDataExchange.StockId);
                }
            }
        }
        ...
        private void PutOrderOnHold_ExecuteCode(object sender, EventArgs e)
        {
            if (orderDataExchange.Client.GroupId == ClientGroupSettings.OxbridgePropertyGroup)
            {
                if (orderDataExchange.OrderId > 0)
                {
                    ...
                    OrderProcessor.GenerateEmailOxbridgeOrderOnHold(orderDataExchange.OrderId, orderDataExchange.PropertyOrder.SendProofTo);
                ...
                if (orderDataExchange.PhotoOrderId > 0)
                {
                    ...
                    OrderProcessor.GenerateEmailOxbridgeOrderOnHold(orderDataExchange.PhotoOrderId, orderDataExchange.PropertyOrder.SendProofTo);
                ...
                if (orderDataExchange.StockId > 0)
                {
                    ...
                    OrderProcessor.GenerateEmailOxbridgeOrderOnHold(orderDataExchange.StockId, orderDataExchange.PropertyOrder.SendProofTo);
        ...
        private void SaveB2BOrderInfo_ExecuteCode(object sender, EventArgs e)
        {
            if (orderDataExchange.PropertyOrder.IsB2BOrder)
            {
                if (orderDataExchange.OrderId > 0)
                    OrderProcessor.GenerateB2BOrderInto(orderDataExchange.OrderId, orderDataExchange.PropertyOrder.BusinessID, orderDataExchange.PropertyOrder.B2bListingID);
                if (orderDataExchange.StockId > 0)
                {
                    if (orderDataExchange.StockId < 99000000)
                        OrderProcessor.GenerateB2BOrderInto(orderDataExchange.StockId, orderDataExchange.PropertyOrder.BusinessID, orderDataExchange.PropertyOrder.B2bListingID);
                }
                if (orderDataExchange.PhotoOrderId > 0)
                    OrderProcessor.GenerateB2BOrderInto(orderDataExchange.PhotoOrderId, orderDataExchange.PropertyOrder.BusinessID, orderDataExchange.PropertyOrder.B2bListingID);
            }
        }
